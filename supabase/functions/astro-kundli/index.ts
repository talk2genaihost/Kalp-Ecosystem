...
function currentDasha(periods:unknown[]): string|null { const now=Date.now(); for(const item of periods){ const p=obj(item); const start=Date.parse(String(p.start??p.startDate??"")); const end=Date.parse(String(p.end??p.endDate??"")); if(Number.isFinite(start)&&Number.isFinite(end)&&now>=start&&now<=end) return scalar(p.maha??p.Maha??p.mahadasha??p.name); } const first=obj(periods[0]); return scalar(first.maha??first.Maha??first.mahadasha??first.name); }
function adaptOpenKundali(raw:unknown){
 ...
 const nak=firstValue(moon,["nakshatra"]); const pada=firstValue(moon,["pada","nakshatraPada"]); const moonSign=firstValue(moon,["sign","rashi","zodiac"]); const sunSign=firstValue(sun,["sign","rashi","zodiac"]);
 const periods=dashaPeriods(data); const yogas=arr(data.yogas).length?arr(data.yogas):arr(data.yogaDetails);
 ...
 return {data,planets,moonSign,sunSign,nakshatra:nak,nakshatraPada:pada,nakshatraLord:firstValue(moon,["nakshatraLord","lord"]),tithi,karana,yoga,mangalDosha:manglik,dasha:currentDasha(periods),dashaPeriods:periods,yogaDetails:yogas};
}
Deno.serve(async(req:Request)=>{
 ...
 try {
   ...
   const a=adaptOpenKundali(providerResponse); const evidence={lagna:"CALCULATED",moonSign:a.moonSign?"PROVIDER":"UNAVAILABLE",sunSign:a.sunSign?"PROVIDER":"UNAVAILABLE",nakshatra:a.nakshatra?"PROVIDER":"UNAVAILABLE",nakshatraPada:a.nakshatraPada?"PROVIDER":"UNAVAILABLE",nakshatraLord:a.nakshatraLord?"PROVIDER":"UNAVAILABLE",tithi:a.tithi?"PROVIDER":"UNAVAILABLE",karana:a.karana?"PROVIDER":"UNAVAILABLE",yoga:a.yoga?"PROVIDER":"UNAVAILABLE",dasha:a.dasha?"PROVIDER":"UNAVAILABLE",dashaPeriods:a.dashaPeriods.length?"PROVIDER":"UNAVAILABLE",mangalDosha:a.mangalDosha?"PROVIDER":"UNAVAILABLE",yogaDetails:a.yogaDetails.length?"PROVIDER":"UNAVAILABLE"};

...
function basePayload(overrides: Record<string, unknown> = {}) {
  return {
    provider: "openkundali",
    adapter: { model: "KALP-KUNDLI-CANONICAL-v1" },
    data: {
      provider: "openkundali",
      lagna: { value: "मकर", degreeText: "13° 33′" },
      moonSign: "Aquarius",
      sunSign: "Scorpio",
      nakshatra: "शतभिषा",
      nakshatraPada: 1,
      ...
      yoga: null,
      mangalDosha: null,
      dasha: { name: "Mercury", mahadasha: "Mercury", antardasha: null, pratyantardasha: null },
      dashaPeriods: [],
      planets: [{ name: "Moon", sign: "Aquarius", degree: 9.1 }],
      yogaDetails: [],
      evidence: {
        lagna: "CALCULATED",
        moonSign: "PROVIDER",
        ...
        nakshatraPada: "CALCULATED",
        nakshatraLord: "CALCULATED",
        tithi: "UNAVAILABLE",
        karana: "UNAVAILABLE",
        yoga: "UNAVAILABLE",
        mangalDosha: "UNAVAILABLE",
        dasha: "PROVIDER",
        dashaPeriods: "UNAVAILABLE",
        yogaDetails: "UNAVAILABLE",
      },
      ...overrides,
    },
  };
...
test("rejects claims for unavailable tithi, karana and mangal dosha", () => {
  ...
  const result = validateInterpretation({
    summary: "सामान्य संकेत।",
    guidance: ["तिथि और करण से विशेष निष्कर्ष निकाला जा सकता है।", "जातक मांगलिक है।"],
  }, evidence);
...
test("allows supplied yoga evidence and evidenced mahadasha", () => {
  ...
  const result = validateInterpretation({
    dasha: ["बुध महादशा के संदर्भ में यह प्रवृत्ति देखी जा सकती है।"],
    yogas: ["प्रदाता द्वारा दिए गए परिवर्तन योग के संदर्भ में संकेत देखे जा सकते हैं।"],
  }, evidence);
